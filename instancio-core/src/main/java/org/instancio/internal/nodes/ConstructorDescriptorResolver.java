...
final class ConstructorDescriptorResolver {
    ...
    ConstructorDescriptor resolve(final InternalNode node, final List<InternalNode> children) {
        return switch (node.getNodeKind()) {
            ...
            default -> throw Fail.withInternalError(
                    "Unexpected NodeKind: {}", node.getNodeKind());
        };
    }

    @Nullable
    private static ConstructorDescriptor resolveForRecord(
    ...
    @Nullable
    private ConstructorDescriptor resolveForPojo(
            final InternalNode node,
            final List<InternalNode> children) {
        ...
        for (InstantiationStrategy strategy : instantiationStrategies.getStrategies()) {
            final ConstructorDescriptor descriptor = switch (strategy) {
                ...
                // null since no constructor is needed/called via BYPASS_CONSTRUCTOR
    ...
    private static ConstructorDescriptor resolveNoArgsConstructor(
        ...
        for (Constructor<?> ctor : targetClass.getDeclaredConstructors()) {
            if (ctor.getParameterCount() == 0 && ctor.trySetAccessible()) {
                return new ConstructorDescriptor(ctor, List.of(), children);
            }
        }
        return null;
    }
