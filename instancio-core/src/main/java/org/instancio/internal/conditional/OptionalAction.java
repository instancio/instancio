...
public class OptionalAction implements Conditional, ConditionalAction {
    ...
    private ConditionalAction addAction(final TargetSelector destination, final GeneratorHolder generatorHolder) {
    ...
    @Override
    public <T> ConditionalAction supply(final TargetSelector selector, final Generator<T> generator) {
        return addAction(selector, GeneratorHolder.of(generator));
    }

    @Override
    public <T> ConditionalAction supply(final TargetSelector selector, final Supplier<T> supplier) {
        return addAction(selector, GeneratorHolder.of(supplier));
    }

    @Override
    public <T> ConditionalAction set(final TargetSelector selector, final T obj) {
        return addAction(selector, GeneratorHolder.of(obj));
    }

    @Override
    public <T> ConditionalAction generate(final TargetSelector selector, final GeneratorSpecProvider<T> gen) {
        return addAction(selector, GeneratorHolder.of(gen));
    }

    @Override
    public <T> ConditionalAction generate(final TargetSelector selector, final GeneratorSpec<T> spec) {
