...
public final class Format {
    ...
    public static String formatMethod(final Method method) {
        final StringBuilder sb = new StringBuilder(SB_SMALL);
        final String annotations = Arrays.stream(method.getDeclaredAnnotations())
                .map(Annotation::annotationType)
                .map(a -> METHOD_PADDING + "@" + Format.withoutPackage(a))
                .collect(joining(NL));
        ...
        return sb.append(METHOD_PADDING).append(Format.withoutPackage(method.getGenericReturnType()))
                .append(' ').append(method.getName())
                .append('(').append(params).append(") {").append(NL)
