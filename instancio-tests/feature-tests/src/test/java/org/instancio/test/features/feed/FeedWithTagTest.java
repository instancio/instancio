...
@FeatureTag({Feature.FEED, Feature.VALUE_SPEC, Feature.GENERATE})
...
class FeedWithTagTest {
    ...
    @Feed.Source(name = "data/FeedWithTag.csv")
    ...
    void usingCustomTagKey(final String id) {
        // Run this first to load a feed with the default tag key.
        // This is to ensure that caching logic takes tag key into account
        // when multiple tag keys are used for the same feed.
        final SampleFeed feedWithDefaultTagKey = Instancio.ofFeed(SampleFeed.class)
                .withTagValue("EN")
                .create();

        assertThat(feedWithDefaultTagKey.field1().get()).matches("D_\\d{3}");
