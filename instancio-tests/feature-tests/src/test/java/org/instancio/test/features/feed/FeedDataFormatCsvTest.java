...
class FeedDataFormatCsvTest {
    ...
    void csvWithTrimmedValue() {
        final boolean trimValues = gen().booleans().get();
        ...
        final String csv = createCsv(commentPrefix, separator);

        final Feed result = Instancio.ofFeed(Feed.class)
                .withDataSource(source -> source.ofString(csv))
