...
@FeatureTag({Feature.SCHEMA, Feature.WITH_SCHEMA, Feature.BLANK})
@ExtendWith(InstancioExtension.class)
class WithSchemaSetBlankTest {

    @SchemaResource(data = "value\nfoo\nbar\nbaz")
    private interface SampleSchema extends Schema {}

    @Test
    void ofBlankWithSchema() {
    ...
    @Test
    void setBlankWithSchema() {
        final Schema schema = Instancio.createSchema(SampleSchema.class);

        final List<StringHolder> result = Instancio.ofList(StringHolder.class)
                .size(3)
                // withSchema() takes precedence over setBlank() regardless of method order
