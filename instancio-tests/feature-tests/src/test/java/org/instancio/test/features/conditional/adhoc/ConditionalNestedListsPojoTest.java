...
class ConditionalNestedListsPojoTest {
    ...
    void fromRootToMidAndInner() {
        List<Root> results = Instancio.ofList(Root.class)
                ...
                .when(valueOf(ROOT_FILED).is("A")
                        .set(MID_FIELD, "A-Mid")
                        .set(INNER_FIELD, "A-Inner"))
                .when(valueOf(ROOT_FILED).is("B")
                        .set(MID_FIELD, "B-Mid")
    ...
    void fromRootToMidAndMidToInner() {
        List<Root> results = Instancio.ofList(Root.class)
                .size(100)
                .generate(ROOT_FILED, gen -> gen.oneOf("A", "B"))
                .when(valueOf(ROOT_FILED).is("A").set(MID_FIELD, "A-Mid"))
                .when(valueOf(ROOT_FILED).is("B").set(MID_FIELD, "B-Mid"))
