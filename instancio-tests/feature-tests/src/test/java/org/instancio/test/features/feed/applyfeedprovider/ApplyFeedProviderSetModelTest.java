...
class ApplyFeedProviderSetModelTest {

    @Test
    void setModel() {
        final Model<FeedWithTag> model = Instancio.of(FeedWithTag.class)
                .applyFeed(root(), feed -> feed
        ...
        final FeedWithTag result = Instancio.of(FeedWithTag.class)
                .setModel(all(FeedWithTag.class), model)
                .create();

        assertThat(result.getId()).isEqualTo(203);
    }
}
