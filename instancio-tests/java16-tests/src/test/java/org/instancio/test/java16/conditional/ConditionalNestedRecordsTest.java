...
class ConditionalNestedRecordsTest {
    ...
    class WhenHThenFWhenFThenDWhenDThenA {
        private final Model<Root> model = Instancio.of(Root.class)
                .generate(field(H::val), gen -> gen.oneOf(H_VAL, OTHER_VAL))
                .when(valueOf(H::val).is(H_VAL).set(field(F::val), F_VAL))
                .when(valueOf(F::val).is(F_VAL).set(field(D::val), D_VAL))
                .when(valueOf(D::val).is(D_VAL).set(field(A::val), A_VAL))
                .toModel();
