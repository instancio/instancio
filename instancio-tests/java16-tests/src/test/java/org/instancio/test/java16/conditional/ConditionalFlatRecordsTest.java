...
class ConditionalFlatRecordsTest {
    ...
    void whenC_thenA_whenA_thenE_whenE_thenB() {
        final Root result = Instancio.of(Root.class)
                ...
                .when(valueOf(C::val).is(C_VAL).set(field(A::val), A_VAL))
                .when(valueOf(A::val).is(A_VAL).set(field(E::val), E_VAL))
                .when(valueOf(E::val).is(E_VAL).set(field(B::val), B_VAL))
                .create();
    ...
    void whenE_thenD_whenD_thenC_whenC_thenB_whenB_thenA() {
        final Root result = Instancio.of(Root.class)
                ...
                .when(valueOf(E::val).is(E_VAL).set(field(D::val), D_VAL))
                .when(valueOf(D::val).is(D_VAL).set(field(C::val), C_VAL))
                .when(valueOf(C::val).is(C_VAL).set(field(B::val), B_VAL))
                .when(valueOf(B::val).is(B_VAL).set(field(A::val), A_VAL))
                .create();
