...
class NodeKindMapResolverTest {

    private final NodeKindResolver resolver = new NodeKindMapResolver();

    @ValueSource(classes = {Map.class, HashMap.class, TreeMap.class})
    @ParameterizedTest
    void verifyMatches(Class<?> klass) {
        assertThat(resolver.resolve(klass)).contains(NodeKind.MAP);
    }

    @ValueSource(classes = {Collection.class, Person.class, int.class, String.class})
    @ParameterizedTest
