...
class NodeKindContainerResolverTest {

    private final NodeKindResolver resolver = new NodeKindContainerResolver(Collections.emptyList());

    @ValueSource(classes = {EnumSet.class, Optional.class})
    @ParameterizedTest
    void verifyMatches(Class<?> klass) {
        assertThat(resolver.resolve(klass)).contains(NodeKind.CONTAINER);
    }

    @ValueSource(classes = {Person.class, int.class, String.class})
    @ParameterizedTest
