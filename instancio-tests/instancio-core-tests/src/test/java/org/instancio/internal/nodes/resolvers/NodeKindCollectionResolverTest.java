...
class NodeKindCollectionResolverTest {

    private final NodeKindResolver resolver = new NodeKindCollectionResolver();

    @ValueSource(classes = {Collection.class, List.class, ArrayList.class, Set.class})
    @ParameterizedTest
    void verifyMatches(Class<?> klass) {
        assertThat(resolver.resolve(klass)).contains(NodeKind.COLLECTION);
    }

    @ValueSource(classes = {Map.class, Person.class, int.class, String.class})
    @ParameterizedTest
