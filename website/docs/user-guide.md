...
```
!!! attention ""
    <lnum>4,7</lnum> The `fill()` method fills the object with random data while preserving the initialized `dateOfBirth` field.<br/>
...
!!! attention ""
    ...
    <lnum>6</lnum> Invoke the `fill()` to fill the object with data.<br/>
...
| `ON_CONSTRUCTOR_ERROR`    | `OnConstructorError`      | `BYPASS_CONSTRUCTOR`                | What should happen if the constructor throws an error |
...
- `BYPASS_CONSTRUCTOR` (default) - bypass the failed constructor, allocating the object
...
`BYPASS_CONSTRUCTOR` does **not** resume the strategy list at the next entry: no other
constructor is attempted. Since allocating an object without a constructor is
what the `BYPASS_CONSTRUCTOR` *strategy* does, `create()` returns `null` if that
strategy is not listed in `INSTANTIATION_STRATEGIES`. For example, with `ALL_ARGS,NO_ARGS`, a throwing all-args
...
You can customize default behavior globally by using an `instancio.properties` file.
Instancio automatically detects and loads this file when it is located at the root of the classpath
(typically `src/test/resources`).

Below is a sample configuration file for reference.
For a comprehensive list of all supported properties and detailed descriptions,
please refer to the {{Keys}} Javadoc.
...
- [Instancio Kotlin](kotlin.md)
- [Instancio Protobuf](protobuf.md)
