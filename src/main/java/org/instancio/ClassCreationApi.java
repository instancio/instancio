...
public class ClassCreationApi<T> implements CreationApi<T> {
    ...
    @Override
    public ClassCreationApi<T> map(Class<?> from, Class<?> to) {
        modelContextBuilder.withSubtypeMapping(from, to);
        return this;
    }

    @Override
    public <V> ClassCreationApi<T> with(String field, ValueGenerator<V> generator) {
        modelContextBuilder.withFieldGenerator(getField(this.rootClass, field), generator);
        return this;
    }

    @Override
    public <V> ClassCreationApi<T> with(Class<?> klass, String field, ValueGenerator<V> generator) {
        modelContextBuilder.withFieldGenerator(getField(klass, field), generator);
        return this;
    }

    @Override
    public <V> ClassCreationApi<T> with(Class<V> klass, ValueGenerator<V> generator) {
